...
- **Base branch: `main`**. Branch off it, and if the base has moved, rebase the single commit onto it so the PR diff stays client-only.

```bash
git checkout -b stack/chore-add-new-client-N origin/main
...
gh pr create --base main --head stack/chore-add-new-client-N \
...
If the previous client-add PR merged while you were working (and its branch was deleted), target `main` directly and rebase your commit onto it so the diff is only the new client.
...
Cross-check against what is actually live in the cluster (the committed `main.tf` can lag behind live — always reconcile against `origin/main`, not a possibly-stale checkout):
...
- Commit message convention for a limit change is a normal `chore:`/`fix:` describing the change (not `chore: add new client`), on base `main`. Do not name the private owner in the PR.
