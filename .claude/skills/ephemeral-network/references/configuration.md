...
If the source is instead the labs submodule inside aztec-packages, its foundation build can rewrite dependencies to `portal:` links outside the Docker context. That repository deliberately excludes the release-image target from its labs build. Use compatible published foundation/toolchain artifacts in a standalone checkout, or a separately verified image-build workflow that packages the required local artifacts. Do not assume a successful `make fast-labs` created a deployable image.
