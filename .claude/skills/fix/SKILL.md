...
1. **Stash & branch**: Stash any uncommitted changes, then create a branch from `main` using the issue's `gitBranchName` from Linear.
   ```
   git stash
...
5. **Push & PR**: Push the branch and create a draft PR.
   ...
   gh pr create --draft --base main --title "fix: ..." --body "..."
   ```
