...
import { DateProvider, Timer, elapsed } from '@aztec-labs/foundation/timer';
...
type SeenCheckpoint = Pick<RejectedCheckpoint, 'checkpointNumber' | 'l1'>;
...
export class ArchiverL1Synchronizer implements Traceable {
  ...
  private async handleCheckpoints(
    ...
    const validation = new PendingChainValidationTracker(initialValidationStatus);
    const blocksAdded: L2Block[] = [];
  ...
  private recordRejected(calldataCheckpoint: RetrievedCheckpointFromCalldata, reason: RejectedCheckpointReason) {
    return this.stores.blocks.addRejectedCheckpoint({
      checkpointNumber: calldataCheckpoint.checkpointNumber,
  ...
  private async rewindSyncPointAfterGap(err: InitialCheckpointNumberNotSequentialError): Promise<void> {
    ...
    const updatedL1SyncPoint =
      previousCheckpoint?.l1.blockNumber ?? lastFinalizedCheckpoint?.l1.blockNumber ?? this.l1Constants.l1StartBlock;
  ...
  private async restoreSyncPointAfterFailedBatch(
    ...
    if (currentSyncPoint !== undefined && currentSyncPoint > restoreTo) {
      ...
      await this.stores.blocks.setSynchedL1BlockNumber(restoreTo);
    }
  }
