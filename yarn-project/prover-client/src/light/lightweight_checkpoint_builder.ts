...
export class LightweightCheckpointBuilder {
   ...
   * bundle to {@link addBlock}, which inserts it into the tree and extends the checkpoint's inbox rolling hash.
  ...
  public async addBlock(
    ...
    // tree's current next-available index). The inbox rolling hash is extended only once the block is fully built
    // (below), so a mid-build failure does not pollute it.
