...
import { maxBy } from '@aztec-labs/foundation/collection';
...
describe('fee quote vs public simulation', () => {
  ...
  const startNodeAt = async (nodeClockSlotStart: bigint) => {
    ...
    node = new AztecNodeService({
      ...
      // The txs under test carry no public calls, so the processor never reaches the AVM.
      avmSimulator: mock<AvmSimulator & AsyncDisposable>(),
    });
  };
