...
describe('NodePublicCallsSimulator', () => {
  ...
  /**
   * Answers the by-number read the next-block message prediction makes for the parent block of an in-progress
   * checkpoint. Only the header's L1-to-L2 message total is read from it.
   */
  ...
  describe('continuing an in-progress checkpoint', () => {
    ...
    it('appends the message bundle the next block would consume', async () => {
      const tx = await lowGasTx();
      setupMidCheckpoint();
      mockCheckpointStartBlockData(BlockNumber(5));
      mockNextL1Slot(SlotNumber(100));
