...
describe('AppendOnlyTreeSnapshot', () => {
  ...
  const indices = [0, 1, 2 ** 32 - 1, 2 ** 32, 2 ** 42, Number.MAX_SAFE_INTEGER].map(TreeLeafIndex);
  ...
  it('parses an index supplied as a decimal string beyond the 32-bit range', () => {
    const index = TreeLeafIndex(2 ** 42);
  ...
  it('serializes to the size it reports', () => {
    const snapshot = new AppendOnlyTreeSnapshot(root, TreeLeafIndex(2 ** 42));
  ...
  it.each([-1, 1.5, Number.MAX_SAFE_INTEGER + 2, NaN, Infinity])('rejects invalid index %p', index => {
    expect(() => TreeLeafIndex(index)).toThrow();
    expect(() => new AppendOnlyTreeSnapshot(root, index as TreeLeafIndex)).toThrow();
  });
