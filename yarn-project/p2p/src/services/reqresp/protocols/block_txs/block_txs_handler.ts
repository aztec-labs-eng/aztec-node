...
export function reqRespBlockTxsHandler(
  ...
  return async (_peerId: PeerId, msg: Buffer) => {
    ...
    // otherwise a peer could name a huge list of hashes (with an empty bit list) and force
    // one response to serialize a full tx per hash. Honest requesters stay under this cap.
