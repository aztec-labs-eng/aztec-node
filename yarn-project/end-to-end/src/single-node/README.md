...
| `partial-proofs/` | Manually driven partial-proof submission. `single_root` (the prover node's `startProof` path on a single root) and `multi_root` (three partial-proof roots are staged and messages consume against any covering root, exercising the multi-root Outbox semantics), and `partial_then_full` (a partial proof held in progress while its epoch ends; the full proof publishes and the partial is superseded). |
