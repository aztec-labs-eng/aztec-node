...
2. **`proposeCheckpoint()`** (blocking, returns a `CheckpointProposal`):
   ...
   - Resolves the streaming Inbox consumption cursor (the parent checkpoint's consumed message position, from the fork's L1→L2 leaf count), and collects `previousCheckpointOutHashes` for prior checkpoints in the same epoch. Each block then consumes every message the local archiver has observed past the cursor, up to the per-block and per-checkpoint caps, with no L1 call (AZIP-22 Fast Inbox). Once a greedy step would cross the last bucket-sized portion of the checkpoint's message capacity (start + 768 with the protocol caps), the checkpoint enters message completion: one `Inbox.getBucketAtOrBeforeTotal` read resolves the live bucket end the remaining scheduled blocks can reach, the range up to it is authenticated against the local log, and consumption is frozen once that target is reached. The final block must end at such a live endpoint or the checkpoint is abandoned.
