...
2. **`proposeCheckpoint()`** (blocking, returns a `CheckpointProposal`):
   ...
   - Resolves the streaming Inbox consumption cursor (the parent checkpoint's consumed message position, from the fork's L1→L2 leaf count), and collects `previousCheckpointOutHashes` for prior checkpoints in the same epoch. Each block then consumes every message the local archiver has observed past the cursor, up to the per-block and per-checkpoint caps, with no L1 call (AZIP-22 Fast Inbox). Once a greedy step would cross the last bucket-sized portion of the checkpoint's message capacity (start + 768 with the protocol caps), that block resolves an endpoint for itself: one `Inbox.getBucketAtOrBeforeTotal` read returns the newest live bucket end at or below both the locally synced total and the checkpoint cap (and, on the checkpoint's final block, also at or below the cursor plus one block's capacity), and the range up to it is authenticated against the local log. Nothing is retained between blocks — the next block resolves again. A block that is not the checkpoint's last one then ends at the further of the resolved endpoint and the safe local step, so consulting L1 never costs it messages, and it may end inside a bucket. The checkpoint's final block instead ends exactly on its resolved endpoint, so it can consume fewer messages than the local log alone would allow; if no live endpoint resolves for it, the checkpoint is abandoned.
   - Runs the integrated `validateCheckpointHeaderAndInbox` preflight before gossip, with the pipelined-parent simulation plan, and again before publication against the real L1 state; the bucket hint the latter returns is the one sent with `propose`. A checkpoint whose blocks the local archiver no longer holds is not published.
...
- `validateCheckpointHeaderAndInbox(header, { expectedTotal, expectedParentCheckpointNumber }, simulationOverridesPlan?)` — eth_call simulation of the Rollup's integrated header and Inbox preflight at the last L1 slot of the checkpoint's L2 slot. Derives the parent from the simulated Rollup storage and checks it is the expected one, runs the shared header checks, resolves the checkpoint's final message total to a live Inbox bucket and applies L1's settlement, cap and censorship rules. Returns the bucket sequence `propose` takes as its hint.
