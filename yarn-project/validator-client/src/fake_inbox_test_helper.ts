...
export type FakeInbox = InboxEndpointReader & {
  /** What each endpoint read asked for, and the L1 height it was pinned to. */
  ...
  /** Runs before each bucket read with its index, so a test can move the ring between two attempts. */
  onRead(hook: (readIndex: number) => void): void;
};

/** The L1 height fake reads are pinned to, unless a test asks for another one. */
...
export function makeFakeInbox(
  ...
  return {
    ...
    client: {
      getBlockNumber: () => (failHead ? Promise.reject(failHead) : Promise.resolve(head)),
    },
