#!/usr/bin/env bash
# Precommit hook for formatting staged noir files in the noir-projects workspaces and keeping
...
staged_nr_files=$(git diff --cached --name-only --diff-filter=d | grep '^noir-projects/.*\.nr$' || true)
# Check for unstaged .nr files too
unstaged_nr_files=$(git diff --name-only --diff-filter=d | grep '^noir-projects/.*\.nr$' || true)
...
if [[ -n "$staged_nr_files" ]]; then
  ...
  for dir in noir-contracts aztec-nr; do
    if [[ -d "$dir" ]]; then
