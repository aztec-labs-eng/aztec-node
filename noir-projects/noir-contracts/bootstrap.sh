...
AZTEC_NR_HASH=${AZTEC_NR_HASH:-$($ROOT/noir-projects/aztec-nr/bootstrap.sh hash)}
...
function bench_cmds {
  ...
  local hash=$(hash_str \
    ...
    $(cache_content_hash \
      ...
      "^noir-projects/noir-contracts/scripts/bench_artifact_sizes.sh"))
  echo "$hash noir-projects/noir-contracts/scripts/bench_artifact_sizes.sh"
}
...
case "$cmd" in
  ...
  "pin-standard-build")
    ...
    ;;
  "hash")
    hash_str $AZTEC_TOOLCHAIN_HASH $AZTEC_NR_HASH $(cache_content_hash "^noir-projects/noir-contracts/")
    ;;
  *)
    default_cmd_handler "$@"
    ;;
